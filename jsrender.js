...
function compileTmpl(name, tmpl, parentTmpl, options) {
   ...
   var isContinue = false;
    do{   
    	var n = tmpl.indexOf("{{for",startIndex);          
    	isContinue = true;
      	if(n > 0)
      	{
       		var var1 = tmpl.indexOf(">",n);
            //var var2 = tmpl.indexOf("{{for", tmpl.indexOf("}}",n)+2);           
            //if(var1<var2 || var2==-1 ){
              tmpl=setCharAt(tmpl,var1," {{:DataAttr}} >");                    
              startIndex = tmpl.indexOf(">",n);            
            /*}
            else
            	isContinue = false;*/
	    }
	...
	for (i = 0; i < l; i++) {
		...
		if ("" + node === node) {
			...
			if (tagName === "*") {
				...
				converter = node[1];
			//alert(node[2][0][1][3][0]);
			
			/*	if(node[3][0] ==="'Product'")
					node[2][0]=node[2][0].replace(/>/, "  data-tds-product='Sagar'>");
				else if(node[3][0] ==="'Options'")	
					node[2][0]=node[2][0].replace(/>/, "  data-tds-option='Ashok'>");
				else if(node[3][0] ==="'Feature'")
					node[2][0]=node[2][0].replace(/>/, "  data-tds-feature='Narayane'>");	*/				
				...
				// if(node[3][0] ==="'Product'")
					// markup=markup.replace(/>/, "  data-tds-product='Sagar'>");
				// else if(node[3][0] ==="'Option'")	
					// markup=markup.replace(/>/, "  data-tds-option='Ashok'>");
				// else if(node[3][0] ==="'Feature'")
				// markup=markup.replace(/>/, "  data-tds-feature='Narayane'>");	
				 // node[8]=markup;
